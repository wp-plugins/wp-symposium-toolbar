...
function symposium_toolbar_add_help_text( $contextual_help, $screen_id, $screen ) {
	...
	case 'appearance_page_wp-symposium-toolbar/wp-symposium-toolbar_admin' :
	case 'wp-symposium_page_wp-symposium-toolbar/wp-symposium-toolbar_admin' :
		...
		if ( WPST_IS_WPS_AVAILABLE ) $help_content .= '<br />6. ' . __('Share WP Symposium features from one site with all sites, from the "WP Symposium" tab on all sites where WP Symposium is activated', 'wp-symposium-toolbar');
		...
		if ( WPST_IS_NETWORK_ADMIN ) get_current_screen()->add_help_tab( array(
			'id'      => 'wpst_network',
		...
		$help_content .= '<p>' . __( 'You may also add WP Symposium notifications to the Toolbar for your members. These will warn the member of any incoming friend request or mail. When there are no new friend requests or mails, the number of friends and stored mails is displayed.', 'wp-symposium-toolbar' ) . '  ' . __('You may decide to show them only when a new event occurs, like a new mail or a new friendship. The behaviour will then be that of the WordPress Updates icon as opposed to the Comments icon which is always displayed by default.', 'wp-symposium-toolbar' ) . '</p>';
		if ( is_multisite() ) $help_content .= '<p>' . __( 'On a multisite network, Site Admins also have the option to display these notification icons accross the whole network, so that the users of their site can access their WPS profile and mail from any site where the Toolbar is shown', 'wp-symposium-toolbar' ) . '.</p>';
		...
		if ( !in_array( 'wps', $wpst_hidden_tabs ) && WPST_IS_WPS_ACTIVE ) get_current_screen()->add_help_tab( array(
		...
		// Share Icons
		$help_content = '<p>' .__( 'At this tab, you may choose to display share icons in your Toolbar, along with links to your site. These icons will be displayed in the frontend only, to all users.', 'wp-symposium-toolbar' ) . '</p>';
		$help_content .= '<p>' . __( 'The first settings let you choose which icons you want to make available in your Toolbar, and which URL should be shared.  Depending on your site content and privacy you will want to share the homepage or its pages individually.', 'wp-symposium-toolbar' ) . '</p>';
		$help_content .= '<p>' . __( 'The other settings determine how these icons will look like.  When using their brand colors, icons will not change their color on mouse hover: you may then choose other style settings on hover.', 'wp-symposium-toolbar' ) . '</p>';
		
		if ( !in_array( 'share', $wpst_hidden_tabs ) ) get_current_screen()->add_help_tab( array(
			'id'      => 'wpst_share',
			'title'   => __('Share', 'wp-symposium-toolbar'),
			'content' => $help_content
		) );
		...
		if ( !in_array( 'style', $wpst_hidden_tabs ) ) get_current_screen()->add_help_tab( array(
			...
			'content' =>
			'<p>' . __('The Styles are split into several boxes that will allow you to customize the Toolbar and the Dropdown Menus, their containers, their items, both normal and hover/focus styles.', 'wp-symposium-toolbar') . '</p>' .
	...
	case 'nav-menus' :
		...
		if ( !in_array( 'menus', $wpst_hidden_tabs ) && WPST_IS_WPS_ACTIVE ) {
			global $wpst_menus;
			$help_content =
