...
function symposium_toolbar_add_help_text( $contextual_help, $screen_id, $screen ) { 
	...
	case 'wp-symposium_page_wp-symposium-toolbar/wp-symposium-toolbar_admin' :
		...
		$content = '<p>' . __('The WP Toolbar is the area of the screen just above a WordPress site that contains useful links to the administration part of the site, the content management, as well as the user account. It is displayed both in the backend (this dashboard) and the frontend (public part) of the site. Whenever the user scrolls down the page, the Toolbar remains visible, providing a quick access to these links.', 'wp-symposium-toolbar') . ' ' . __('This plugin allows you to change the default behaviour of the WP Toolbar, and customize its display. For the Toolbar itself, and most of its items, you may select roles for which the item should be displayed.', 'wp-symposium-toolbar') . '</p>' .
		$content .= '<p>' . __('This Options page is split into several parts.', 'wp-symposium-toolbar') . ' ' . __('The first set of options will let you choose which of the WP default items, menus, icons and links to show to users with the appropriate rights.', 'wp-symposium-toolbar') . ' ';
		if ( WPS_TOOLBAR_USES_WPS ) $content .= __('The plugin adds items specific to WP Symposium, that you may show or hide from the second section.', 'wp-symposium-toolbar') . ' ';
		$content .= __('Next section, you may change the content of the WP User Menu, and pick the items you want to display to all users, registrered and logged-in.', 'wp-symposium-toolbar') . ' ' .
		...
		$content .= '<p>' . __('Please refer to these help tabs for a thorough description of the options and tips.', 'wp-symposium-toolbar') . ' ' ;
		if ( WPS_TOOLBAR_USES_WPS ) $content .= __('Please also refer to the help tab added to the WP NavMenus settings page, when creating your menus with WP Symposium items.', 'wp-symposium-toolbar') . '</p>';
		...
		if ( WPS_TOOLBAR_USES_WPS ) get_current_screen()->add_help_tab( array(
			'id'      => 'wp_symposium',
	...
	case 'nav-menus' :
		...
		if ( WPS_TOOLBAR_USES_WPS ) {
			...
			foreach ( $wpst_menus["WPS Profile"] as $profile ) {
				if ( ( $profile[2] != '' ) ) {
						$content .= '<tr>';
							$content .= '<td>'.__($profile[0], WPS_TEXT_DOMAIN).'</td>';
							$content .= '<td>'.$profile[2].'</td>';
							$content .= '<td>'.__($profile[3], 'wp-symposium-toolbar').'</td>';
						$content .= '</tr>';
				}
			}
			$content .=	'</tr>'.
